...
def list_wd_excel(study_number, room_id, list_l, wenshidu, s_datetime, t_datetime):
    '''
    输入列表转换为excel温度
    '''
    st1 = (26, 16)  # 兔狗猴 1F 3F 4F
    st2 = (29, 18)  # 豚鼠
    st3 = (26, 20)  # SPF 2F
    wb = Workbook()  # 实例化
    ws = wb.active  # 激活一个sheet 默认0 sheet
    Table_Title_A = 'Time'
    Table_Title_B = '{}{}趋势'.format(room_id, wenshidu)
    x_filename = '{}-{}{}-{}至{}.xlsx'.format(study_number, room_id, wenshidu, s_datetime, t_datetime)
    Title_width = 22.00
    size_l = len(list_l)
    size_l_x = size_l + 1
    ws.cell(row=1, column=1, value=Table_Title_A)
    ws.cell(row=1, column=2, value=Table_Title_B)
    for i in ['A', 'B']:  # 设定列宽
        ws.column_dimensions[i].width = Title_width

    for y, x in enumerate(range(2, size_l_x + 1)):
        ws.cell(row=x, column=1, value=list_l[y][1])
        ws.cell(row=x, column=2, value=float(list_l[y][2]))
        ws.cell(row=x, column=2).number_format = '0.0'
    '''
    最高值    最低值    不合格    合格    合格率（%）
    '''
    room_id_2 = int(room_id[1:2])  # 根据房间编号判断是第几层
    ws.cell(row=size_l_x + 1, column=1, value='最高值')
    ws.cell(row=size_l_x + 1, column=2, value='=MAX(B{}:B{})'.format(2, size_l_x))
    ws.cell(row=size_l_x + 2, column=1, value='最低值')
    ws.cell(row=size_l_x + 2, column=2, value='=MIN(B{}:B{})'.format(2, size_l_x))
    ws.cell(row=size_l_x + 3, column=1, value='不合格')
    if room_id in ['1151', '1152', '1153']:
        ws.cell(row=size_l_x + 3, column=2, value='=COUNTIF(B{}:B{},">{}")'.format(2, size_l_x, st2[0]))
        ws.cell(row=size_l_x + 4, column=2, value='=COUNTIF(B{}:B{},"<{}")'.format(2, size_l_x, st2[1]))
    elif room_id_2 == 2:
        ws.cell(row=size_l_x + 3, column=2, value='=COUNTIF(B{}:B{},">{}")'.format(2, size_l_x, st3[0]))
        ws.cell(row=size_l_x + 4, column=2, value='=COUNTIF(B{}:B{},"<{}")'.format(2, size_l_x, st3[1]))
    elif room_id_2 in [1, 3, 4]:
        ws.cell(row=size_l_x + 3, column=2, value='=COUNTIF(B{}:B{},">{}")'.format(2, size_l_x, st1[0]))
        ws.cell(row=size_l_x + 4, column=2, value='=COUNTIF(B{}:B{},"<{}")'.format(2, size_l_x, st1[1]))

    '''合并单元格'''
    ws.merge_cells(start_row=size_l_x + 3, start_column=1, end_row=size_l_x + 4, end_column=1)
    ws.cell(row=size_l_x + 5, column=1, value='合格')
    ws.cell(row=size_l_x + 5, column=2, value='={}-(B{}+B{})'.format(size_l, size_l_x + 3, size_l_x + 4))
    ws.cell(row=size_l_x + 6, column=1, value='合格率（%）')
    ws.cell(row=size_l_x + 6, column=2, value='=B{}/{}*100'.format(size_l_x + 5, size_l))
    ws.cell(row=size_l_x + 6, column=2).number_format = '0.00'

    '''日最大值日最小值日温差'''
    num96 = [num for num in range(97, size_l+2, 96)]
    for i in num96:
        ws.cell(row=i, column=3, value='日最大值')
        ws.cell(row=i, column=4, value='=MAX(B{}:B{})'.format(i-95, i))
        ws.cell(row=i, column=5, value='日最小值')
        ws.cell(row=i, column=6, value='=MIN(B{}:B{})'.format(i-95, i))
        ws.cell(row=i, column=7, value='日温差')
        ws.cell(row=i, column=8, value='=D{}-F{}'.format(i, i))
    ws.cell(row=num96[-1] + 6, column=8, value='=MAX(H2:H{})'.format(num96[-1]))
    ws.cell(row=num96[-1] + 5, column=8, value='=MIN(H2:H{})'.format(num96[-1]))

    for i in ws.rows:  # 设定所有居中显示及线框
        for k in i:
            k.alignment = Alig_center
            k.border = border_All

    '''异常数据sheet设定'''
    ws1 = wb.create_sheet(title='{}异常数据'.format(wenshidu), index=2)
    ws1.cell(row=1, column=1, value=Table_Title_A)
    ws1.cell(row=1, column=2, value=Table_Title_B)
    if room_id in ['1151', '1152', '1153']:
        er_l = [(x, y, z) for x, y, z in list_l if float(z) > float(st2[0]) or float(z) < float(st2[1])]
    elif room_id_2 == 2:
        er_l = [(x, y, z) for x, y, z in list_l if float(z) > float(st3[0]) or float(z) < float(st3[1])]
    elif room_id_2 in [1, 3, 4]:
        er_l = [(x, y, z) for x, y, z in list_l if float(z) > float(st1[0]) or float(z) < float(st1[1])]
    for y, x in enumerate(range(2, len(er_l) + 2)):
        ws1.cell(row=x, column=1, value=er_l[y][1])
        ws1.cell(row=x, column=2, value=float(er_l[y][2]))
        ws1.cell(row=x, column=2).number_format = '0.0'
    '''保存'''
    wb.save(x_filename)


def list_sd_excel(study_number, room_id, list_l, wenshidu, s_datetime, t_datetime):
    '''
    输入列表转换为excel湿度
    '''
    ...
    ws.cell(row=size_l_x + 2, column=2, value='=MIN(B{}:B{})'.format(2, size_l_x))
    ws.cell(row=size_l_x + 3, column=1, value='不合格')
    ws.cell(row=size_l_x + 3, column=2, value='=COUNTIF(B{}:B{},">70")'.format(2, size_l_x))
    ws.cell(row=size_l_x + 4, column=2, value='=COUNTIF(B{}:B{},"<40")'.format(2, size_l_x))
    '''合并单元格'''
    ws.merge_cells(start_row=size_l_x + 3, start_column=1, end_row=size_l_x + 4, end_column=1)
    ws.cell(row=size_l_x + 5, column=1, value='合格')
    ws.cell(row=size_l_x + 5, column=2, value='={}-(B{}+B{})'.format(size_l, size_l_x + 3, size_l_x + 4))
    ...
    ws1.cell(row=1, column=1, value=Table_Title_A)
    ws1.cell(row=1, column=2, value=Table_Title_B)
    er_l = [(x, y, z) for x, y, z in list_l if float(z) > float(70) or float(z) < float(40)]
    for y, x in enumerate(range(2, len(er_l) + 2)):
        ws1.cell(row=x, column=1, value=er_l[y][1])
        ws1.cell(row=x, column=2, value=float(er_l[y][2]))
        ws1.cell(row=x, column=2).number_format = '0.0'
...
def Creat_docx(study_number, study_name, study_anmail, room_id, s_datetime, t_datetime, rep):
    word_path_moban = r'K:\mashuaifei\温湿度导出py\moban.docx'
    ...
    doc.save(new_docx)
    '''替换word中的名字部分'''
    namedocx = os.path.join(r'K:\mashuaifei\温湿度导出py', new_docx)
    ...
    '''替换页眉'''
    old = namedocx
    new = r'K:\mashuaifei\温湿度导出py\\'+ study_number + 'ok.docx'
    print(old)
    print(new)
    docx_replace(old, new, rep)


def jangs_excel(study_number, room_id, s_datetime, t_datetime):
    wd = '温度'
    sd = '湿度'
    ...
    rew_list, res_list = wsd_query(wd_pid, sd_pid, data0, data1)
    list_wd_excel(study_number, room_id, rew_list, wd, s_datetime, t_datetime)
    list_sd_excel(study_number, room_id, res_list, sd, s_datetime, t_datetime)
    print('导出成功')


if __name__ == "__main__":
    # study_number = input('请输入实验编号:')
    study_number = 'A2017004-T011-01'
    rep = {'studynum': study_number}
    # study_name = input('请输入实验名称:')
    study_name = 'SD大鼠皮下注射给予TJ101 5周免疫毒性试验'
    study_anmail = 'SD大鼠'
    # room_id = input([房间ID:','房间ID:'])
    room_id = ['1245A', '1253A','1265A' , '1258A']
    # s_datetime = input('起始日期:举例(20190725)')
    s_datetime = ['20170609', '20170713','20171113', '20171113']
    # 2017 09 22至 2017 11 13
    # 2017 11 14至 2017 12 11
    # t_datetime = input('结束日期:举例(20190725)')
    t_datetime = ['20171113', '20171113','20171215','20180112']
    ...
    # PQ_out('20190806', '20190808')
    # PQ_out_all('20190806', '20190808')
